...
function renderBankMatrix(container, banks, metrics) {
  const m = new Map((metrics.metrics || []).map(x => [x.bank_code, x]));
  container.innerHTML = `
        ...
        <table class="matrix-table">
          ...
          <tbody>
            ${banks.map(b=>{
              ...
              const caps = b.supported_formats ? b.supported_formats.map(fmt => 
                `<span class="cap cap-on">${fmt.toUpperCase()}</span>`
              ).join(' ') : '';
              const sr = typeof mm.success_rate === 'number' ? (mm.success_rate*100).toFixed(0)+'%' : '-';
              const btn = (!b.enabled || b.circuit_open)
                ? `<button class="btn btn-outline" data-bank="${b.bank_code}" data-action="use-template" data-i18n="use_csv_template">Use CSV Template</button>`
                : `<button class="btn btn-primary" data-bank="${b.bank_code}" data-action="select-bank" data-i18n="choose_this_bank">Choose This Bank</button>`;
              return `
                <tr>
                  <td>${b.bank_name_en || b.bank_code}</td>
                  <td>${caps}</td>
                  <td>${statusBadge}</td>
                  <td>${sr}</td>
                  <td>${btn}</td>
...
export async function initBankSupportMatrix({ mountSelector, selectElSelector }) {
  try {
    ...
    const banks = Array.isArray(sup) ? sup : (sup.banks || []);
    ...
    if (select) {
      ...
      banks.forEach(b=>{
        ...
        const label = `${b.bank_name_en || b.bank_code}${b.circuit_open ? ' (⚡)' : ''}${!b.enabled ? ' (X)' : ''}`;
      ...
      container.addEventListener('click', (e)=>{
        ...
        if (!btn) return;
        const bankCode = btn.dataset.bank;
