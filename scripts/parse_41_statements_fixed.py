#!/usr/bin/env python3
"""
完整解析Cheok Jun Yoon的41份信用卡账单 - 修复版
修复: Decimal JSON序列化问题
"""
import sys
import os
import json
...
def parse_bank(bank_name, pdf_files, doc_ai, parser):
    ...
    for idx, pdf_path in enumerate(pdf_files, 1):
        ...
        try:
            ...
            print(f"  ❌ 解析失败: {str(e)}")
            results.append({'file': filename, 'month': month, 'bank': bank_name, 'error': str(e)})
    
    # 保存银行结果到文件（使用Decimal处理）
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump({
            ...
            'results': results
        }, f, indent=2, ensure_ascii=False, default=decimal_default)
