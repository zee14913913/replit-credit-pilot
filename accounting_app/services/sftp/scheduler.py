...
class SFTPScheduler:
    ...
    def __init__(self, company_id: int = 1, sync_interval_minutes: int = 10):
        ...
        logger.info(
            f"⏰ SFTP Scheduler initialized (sync every {sync_interval_minutes} minutes)"
        )
    ...
    def _sync_task(self):
        ...
        try:
            logger.info(
                f"⏰ [Scheduled Sync] Starting at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
            )
            ...
            try:
                # 执行同步
                sync_service = SFTPSyncService(db_session=db,
                                               company_id=self.company_id)
                result = sync_service.scan_and_upload_files(
                    is_manual=False, uploaded_by="scheduler")

                logger.info(
                    f"✅ [Scheduled Sync] Completed: {result['uploaded']} uploaded, "
    ...
    def _retry_task(self):
        ...
        try:
            logger.info(
                f"🔄 [Scheduled Retry] Starting at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
            )
            ...
            try:
                ...
                if result['retried'] > 0:
                    logger.info(
                        f"✅ [Scheduled Retry] Completed: {result['succeeded']} succeeded, "
                        f"{result['failed']} failed")
                else:
                    logger.debug(
                        f"ℹ️  [Scheduled Retry] No failed uploads to retry")
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "next_run":
            str(schedule.next_run()) if schedule.get_jobs() else None
        }
...
def get_scheduler(company_id: int = 1,
                  sync_interval_minutes: int = 10) -> SFTPScheduler:
    """
    获取全局调度器实例（单例模式）
    ...
    if _global_scheduler is None:
        _global_scheduler = SFTPScheduler(
            company_id=company_id, sync_interval_minutes=sync_interval_minutes)

    return _global_scheduler


def start_global_scheduler(company_id: int = 1,
