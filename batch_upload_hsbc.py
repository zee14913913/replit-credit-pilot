...
def batch_upload_hsbc_statements():
    ...
    # 获取所有新转换的HSBC PDF文件
    pdf_files = sorted(glob.glob("attached_assets/*Statement_176046*.pdf"))
    
    print("="*80)
    print(f" HSBC批量处理 - 共 {len(pdf_files)} 个账单")
    print("="*80)
    ...
    for i, pdf_path in enumerate(pdf_files, 1):
        ...
        try:
            ...
            print(f"    ✅ 解析成功: {stmt_date} | 交易 {len(transactions)} 笔 | 总额 RM {stmt_total:,.2f}")
