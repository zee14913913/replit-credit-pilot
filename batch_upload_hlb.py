#!/usr/bin/env python3
"""
Batch upload script for Chang Choon Chow's Hong Leong Bank statements
...
from ingest.statement_parser import parse_hong_leong_statement

def extract_statement_date_from_filename(filename):
    """Extract statement date from filename like '07-01-2025_*.pdf'"""
    match = re.match(r'07-(\d{2})-(\d{4})', filename)
    if match:
        month = match.group(1)
        year = match.group(2)
        # Statement date is the 7th of the month
        return f"{year}-{month}-07"
    return None
...
def main():
    ...
    cursor.execute(
        """SELECT id, card_number_last4, card_type 
           FROM credit_cards 
           WHERE customer_id = ? AND bank_name = 'Hong Leong Bank'""",
        (customer_id,)
    )
    cards = {row[1]: (row[0], row[2]) for row in cursor.fetchall()}
    
    print(f"\n💳 Available HLB cards:")
    for last4, (card_id, card_type) in cards.items():
        print(f"   Card {card_id}: {card_type} (*{last4})")
    
    # List statement files
    upload_dir = "static/uploads"
    statement_files = sorted([
        f for f in os.listdir(upload_dir) 
        if f.startswith("07-") and f.endswith(".pdf")
    ])
    ...
    for filename in statement_files:
        ...
        # Parse the statement
        info, transactions = parse_hong_leong_statement(file_path)
        ...
        # Get card info from parsed data
        card_last4 = info.get('card_last4', '')
        if not card_last4 or card_last4 not in cards:
            # Default to SUTERA card (2033) if not found
            card_last4 = '2033'
            print(f"⚠️  Using default card (*{card_last4})")
        
        card_id, card_type = cards[card_last4]
        print(f"💳 Card: {card_type} (*{card_last4})")
