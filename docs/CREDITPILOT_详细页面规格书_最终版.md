...
系统会将各维度的近期趋势合成综合评分，并以A/B/C/D等级呈现，帮助用户判断当前财务状态与改善重点。评分引擎使用内部基准进行综合评估。
...
*注：系统综合四个维度生成A-F评级，卡片中显示为示例说明*

### 【如何善用】
1. **每月追踪**: 观察评级趋势，及早发现问题
2. **申贷前检查**: C级以下先改善再申贷
3. **对标改善**: 查看各项细分，针对性提升
...
# 匹配结果示例
卡号匹配: ✅ 完全一致（最高优先级，权重40%）
日期匹配: ✅ 同一天（权重30%）
金额匹配: ✅ 完全一致（权重20%）
商户匹配: 🟡 高度相似（权重10%，KFC PAVILION vs KFC PAVILION KL）
...
1. **账单验证（双重验证系统）**
   - Level 1: PDF总额交叉验证（提取官方Total Debit/Credit）
   - Level 2: 信心评分系统（基于关键词长度）
   - 状态：通过 / 需复核

2. **收据覆盖率**
   - 已匹配交易数 / 总交易数 × 100%
   - 95%+为优秀，80-95%为良好，60-80%为尚可
   - 示例：38笔已匹配 / 45笔总交易 = 84.4% → 良好
...
系统综合三个维度的表现生成A/B/C/D评级，帮助用户了解数据质量并提供改善建议。
...
# 收据覆盖率计算
def calculate_receipt_coverage():
    已匹配数 = COUNT(transactions WHERE receipt_id IS NOT NULL)
    总交易数 = COUNT(transactions)
    覆盖率 = 已匹配数 / 总交易数 × 100%
...
*注：系统内部使用综合评估机制生成A/B/C/D评级，上述示例为技术实现说明*
