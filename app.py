...
def export_transaction_image(transaction_id):
    ...
    # 创建图片 (1200x400像素)
    img = Image.new('RGB', (1200, 400), color='#000000')
    ...
    # 绘制边框
    draw.rectangle([(10, 10), (1190, 390)], outline='#FF007F', width=3)
    
    # 标题
    draw.text((40, 30), "TRANSACTION RECORD / 交易记录", fill='#FF007F', font=title_font)
    
    # 交易类型颜色
    trans_color = '#FF007F' if trans['transaction_type'] == 'debit' else '#00FF9D'
    trans_sign = '-' if trans['transaction_type'] == 'debit' else '+'
    ...
    # 绘制信息
    fields = [
        ("Date / 日期:", str(trans['transaction_date'])),
        ("Bank / 银行:", trans['bank_name']),
        ("Account / 账号:", f"****{trans['account_number_last4']}"),
        ("Description / 描述:", trans['description'][:60]),
        ("Amount / 金额:", f"{trans_sign}RM {abs(trans['amount']):,.2f}"),
        ("Customer Tag / 客户:", trans['customer_name_tag'] or '-'),
    ]
    
    for label, value in fields:
        draw.text((40, y), label, fill='#C0C0C0', font=label_font)
        color = trans_color if 'Amount' in label else '#FFFFFF'
        draw.text((280, y), value, fill=color, font=value_font)
        y += line_height
    
    # 添加底部标识
    draw.text((40, 350), "Generated by INFINITE GZ Smart Credit Manager", fill='#322446', font=label_font)
    ...
    return send_file(img_io, mimetype='image/png', as_attachment=True, 
                    download_name=f'transaction_{transaction_id}_{trans["transaction_date"]}.png')
